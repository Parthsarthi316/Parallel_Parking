...
# import HybridAstarPlanner.drawdiwheel as draw
...
class C:  # Parameter config
    ...
    N_STEER = 5.0  # steer command number
    ...
    RF = 1.5  # [m] distance from rear to vehicle front end of vehicle
...
def calc_next_node(n_curr, c_id, u, d, P): # Calculate next node
    step = C.XY_RESO * 2
...
def analystic_expantion(node, ngoal, P): # using reeds_shepp to calculate paths
...
def draw_car(x, y, yaw, steer, color='black'):
    ...
    diend1,diend2= [items[-1] for items in car]
    plt.plot(car[0, :], car[1, :], color='green')
...
def design_obstacles(x, y):
    ...
    a=20
    b=31
    for i in range(x):# top edge of map
        ox.append(i)
        oy.append(0)
    for i in range(x):# bottom edge of map
        ox.append(i)
        oy.append(y - 1)
    for i in range(y):# left edge of map
        ox.append(0)
        oy.append(i)
    for i in range(y):# right edge of map
        ox.append(x - 1)
        oy.append(i)
    for i in range(a,b):#inner obstacle top
        ox.append(i)
        oy.append(a)
    for i in range(a,b):#inner obstacle bottom
        ox.append(i)
        oy.append(b - 1)
    for i in range(a,b):#inner obstacle left
        ox.append(a)
        oy.append(i)
    for i in range(a,b):#inner obstacle right
        ox.append(b - 1)
        oy.append(i)
    for i in range(11):#left car right edge
        ox.append(21)
        oy.append(i)
    for i in range(21):#left car top edge
        ox.append(i)
        oy.append(10)
    for i in range(11):#right car left edge
        ox.append(33)
        oy.append(i)
    for i in range(33,51):#right car top edge
        ox.append(i)
        oy.append(10)
...
def main():
    print("start!")
    x,y=51,51 #map size
    sx, sy, syaw0 = 10.0, 40.0, np.deg2rad(0.0) #spawn location and orientation
    gx, gy, gyaw0 = 25.0, 5.0, np.deg2rad(0.0) #goal and orientation
    ...
    for k in range(len(x)):
        ...
        plt.title("Hybrid A* for Diwheel Robot")
        plt.axis("equal")
        plt.pause(0.0001)
