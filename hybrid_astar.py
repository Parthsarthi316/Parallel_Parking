...
def calc_next_node(n_curr, c_id, u, d, P): # Calculate next possible node
    step = C.XY_RESO * 2
...
def design_obstacles(x, y):
    ...
    for i in range(11):#right car left edge
        ox.append(31)
        oy.append(i)
    for i in range(31,51):#right car top edge
        ox.append(i)
        oy.append(10)
...
def main():
    print("start!")
    x,y=51,51 # Map size
    sx, sy, syaw0 = 10.0, 40.0, np.deg2rad(0.0) #Spawn point
    gx, gy, gyaw0 = 25.0, 5.0, np.deg2rad(0.0)  #Goal
