...
class C:  # Parameter config
    ...
    W = 1.75  # [m] width of vehicle
    WB = 3.0  # [m] wheel base: rear to front steer
    ...
    RTR = 3.5  #[m] rear to trailer wheel
    RTF = 1.0  # [m] distance from rear to vehicle front end of trailer
    RTB = 5.0  # [m] distance from rear to vehicle back end of trailer
...
def main():
    ...
    sx, sy = 17.0, 40.0  # Spawn location

    syaw0 = np.deg2rad(0.0)# Spawn Angle for truck
    syawt = np.deg2rad(0.0)# Spawn Angle for trailer

    gx, gy = 40.0, 5.0  # [m] Goal

    gyaw0 = np.deg2rad(0.0) # Goal Orientation for truck
    gyawt = np.deg2rad(0.0) # Goal Orientation for trailer
    x,y=51,51               # Map size
